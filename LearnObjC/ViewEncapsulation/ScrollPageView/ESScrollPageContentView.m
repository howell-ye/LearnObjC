...
@interface ESScrollPageContentView()<UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
{
    CGFloat   _oldOffSetX;
...
// 用于处理重用和内容的显示
@property (strong, nonatomic) ESScrollPageCollectionView *collectionView;
// collectionView的布局
...
// 所有的子控制器
@property (strong, nonatomic) NSMutableDictionary<NSString *, UIViewController<ESScrollPageViewChildVcDelegate> *> *childVcsDic;
// 当前控制器
@property (strong, nonatomic) UIViewController<ESScrollPageViewChildVcDelegate> *currentChildVc;
...
@implementation ESScrollPageContentView
...
- (void)commonInit {
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [_collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(ESScrollPageCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
...
- (void)receiveMemoryWarningHander:(NSNotificationCenter *)noti {
    ...
    __weak typeof(self) weakSelf = self;
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<ESScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf) {
            if (childVc != strongSelf.currentChildVc) {
                [_childVcsDic removeObjectForKey:key];
                [ESScrollPageContentView removeChildVc:childVc];
            }
        }
...
#if DEBUG
    NSLog(@"ESScrollPageContentView---销毁");
#endif
}
...
- (void)reload {
    ...
    [self.childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<ESScrollPageViewDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        [ESScrollPageContentView removeChildVc:childVc];
        childVc = nil;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    // 调整title
    [self adjustBarTitleOffsetToCurrentIndex:currentIndex];
}
...
- (void)contentViewDidMoveFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress {
    if(self.barView) {
        [self.barView adjustUIWithProgress:progress oldIndex:fromIndex currentIndex:toIndex];
    }
}

- (void)adjustBarTitleOffsetToCurrentIndex:(NSInteger)index {
    if(self.barView) {
        [self.barView adjustTitleOffSetToCurrentIndex:index];
    }
}

- (void)willAppearWithIndex:(NSInteger)index {
    UIViewController<ESScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(ESScroll_viewWillAppearForIndex:)]) {
            [controller ESScroll_viewWillAppearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)didAppearWithIndex:(NSInteger)index {
    UIViewController<ESScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(ESScroll_viewDidAppearForIndex:)]) {
            [controller ESScroll_viewDidAppearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)willDisappearWithIndex:(NSInteger)index {
    UIViewController<ESScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(ESScroll_viewWillDisappearForIndex:)]) {
            [controller ESScroll_viewWillDisappearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)didDisappearWithIndex:(NSInteger)index {
    UIViewController<ESScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(ESScroll_viewDidDisappearForIndex:)]) {
            [controller ESScroll_viewDidDisappearForIndex:index];
        }
        if (_needManageLifeCycle) {
            [controller endAppearanceTransition];
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (_currentChildVc == nil) {
            ...
            if (!_currentChildVc || ![_currentChildVc conformsToProtocol:@protocol(ESScrollPageViewChildVcDelegate)]) {
                NSAssert(NO, @"子控制器必须遵守ESScrollPageViewDelegate协议");
            }
            // 设置当前下标
    ...
    if (_currentChildVc.scrollViewController != self.parentViewController) {
    ...
    if (_isLoadFirstView) { // 第一次加载cell? 不会调用endDisplayCell
        ...
        if (isFirstLoaded) {
            // viewDidLoad
            if ([_currentChildVc respondsToSelector:@selector(ESScroll_viewDidLoadForIndex:)]) {
                [_currentChildVc ESScroll_viewDidLoadForIndex:indexPath.row];
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.forbidTouchToAdjustPosition) {
        if (_currentIndex == indexPath.row) {// 没有滚动完成
            ...
            if (_needManageLifeCycle) {
                UIViewController<ESScrollPageViewChildVcDelegate> *currentVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", _oldIndex]];
                // 开始出现
                [currentVc beginAppearanceTransition:YES animated:NO];
                
                UIViewController<ESScrollPageViewChildVcDelegate> *oldVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", indexPath.row]];
                // 开始消失
...
- (ESScrollPageCollectionView *)collectionView {
    if (_collectionView == nil) {
        ESScrollPageCollectionView *collectionView = [[ESScrollPageCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.collectionViewLayout];
        ...
        collectionView.scrollEnabled = self.barView.barStyle.isScrollContentView;
...
- (NSMutableDictionary<NSString *,UIViewController<ESScrollPageViewChildVcDelegate> *> *)childVcsDic {
    if (!_childVcsDic) {
