...
@interface ESColumnChartView()<CAAnimationDelegate>
...
@implementation ESColumnChartView
...
-(void)showAnimation{
    ...
    if (_needXandYLine) {
        ...
        if (self.isShowYLine) {
            ...
            [bezier addLineToPoint:CGPointMake(self.originSize.x, 20)];
        }
        
        [bezier moveToPoint:CGPointMake(self.originSize.x, CGRectGetHeight(self.frame) - self.originSize.y)];
        
        [bezier addLineToPoint:CGPointMake(_maxWidth , CGRectGetHeight(self.frame) - self.originSize.y)];
        ...
        for (NSInteger i = 0; i<5; i++) {
            ...
            [second moveToPoint:CGPointMake(_originSize.x, CGRectGetHeight(self.frame) - _originSize.y -height)];
            [second addLineToPoint:CGPointMake(_maxWidth, CGRectGetHeight(self.frame) - _originSize.y - height)];
...
//                NSValue *lineValue = [NSValue valueWithCGPoint:CGPointMake(CGRectGetMaxX(itemsView.frame) - _columnWidth / 2, CGRectGetHeight(self.frame) - valueFloat * _perHeight - _originSize.y -1)];
